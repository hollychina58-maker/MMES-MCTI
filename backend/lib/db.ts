...
async function restoreProductsFromJson(sql: SqlType): Promise<void> {
  try {
    ...
    // JSON backup file not available (e.g., on Vercel serverless) — database is the source of truth
    console.log('No products JSON backup found, skipping auto-restore');
  }
}
...
async function restorePostsFromJson(sql: SqlType): Promise<void> {
  try {
    ...
    // JSON backup file not available (e.g., on Vercel serverless) — database is the source of truth
    console.log('No blog JSON backup found, skipping auto-restore');
  }
}
